...
const FormFX = function() {
  ...
  formsForm.onkeypress = interceptEnter;
    ...
    switch (inputType) {
      ...
      case "file": {
        ...
        function getThumbSrc(file, seekTo = 5.0) {
          return new Promise((resolve, reject) => {
            ...
            if (file.type.match('image')) {
              ...
              throw new Error('pdf thumbnail generation not supported')
            } else {              
              fileReader.onload = function() {
        ...
        function resolveFromWordpress(response) {
            ...
            summaryInput.querySelector(".summary-list").innerHTML = `
              <ul class="response-files">
                ${response.map(metadata => {
                  return `
                    <li class="response-file" data-id="${metadata.id}" data-thumb="${metadata.source_url || metadata.source_url || ''}">${metadata.originalname}</li>
                  `}
                ).join("\n")}
              </ul>
            `;
          } catch(e){
            console.log("respones error",e,response)
          }
          ...
          // summaryInput.querySelector("input[type='hidden']").value = response.map(item => item.id);
          const fileSummary = summaryInput.querySelector(".filesummary");
          response.map(item => item.id).forEach(function(imageID){
            const hiddenInput = document.createElement("input");
            hiddenInput.type = "hidden";
            hiddenInput.name = fileSummary.dataset.name;
            hiddenInput.value = imageID;
            fileSummary.appendChild(hiddenInput);
          });
  ...
  function interceptEnter(e) {
    const elem = e.target;
    if (e.code === "Enter" && elem.matches("input")) {
      elem.blur();
      return false;
    } else {
      return true;
    ...
    reqRads.forEach(function(reqElem) {
      ...
      idsArray.forEach(function(radioID){
        console.log(radioID, document.getElementById(radioID));
  ...
  function handleDatalist(e) {
    ...
    if ([...this.list.options].map(option => option.value).includes(listValue) || this.matches("[data-customvalue = 'allowed']")) {
	...
	async function validateAndSubmit(e) {
      ...
      try {
        editUrl = document.querySelector("#editUrl").value  
      } catch(e) {
        console.log("not in edit mode, likely")
      }
      const submissionUrl = editUrl ? editUrl : "/wp/formData"
  ...
  function handleSubmissionResponse(json) {
    ...
    If you need to submit additional items, please <a href="${window.location}">fill out this form again</a>.
    <a class="button" href="${window.location}">Reload form</button>
    `;
	...
	function isValid(thisInput) {
      ...
      case "datalist":
        ...
        break;
			...
			default:
				const inputFilled = thisInput.querySelector("input") ? thisInput.querySelector("input").value.length : 0;
				if (inputFilled === 0) {
					isValid = false;
				}
		}
  ...
  (function initPage() {
    handleReqRads(); // Should show/hide sections appropriately, if radio buttons are pre-selected at page load time
  })();
};
